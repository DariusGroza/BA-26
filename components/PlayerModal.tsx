...
const PlayerModal: React.FC<PlayerModalProps> = ({ 
  ...
  const getTrainingCost = (val: number) => {
    if (val >= 85) return 25;
    if (val >= 70) return 10;
    return 5;
  };
  ...
  return (
    <div className="fixed inset-0 z-[75] flex items-center justify-center p-2 md:p-4 animate-in fade-in duration-200">
      ...
      <div className="relative bg-[#111114] border border-white/10 w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95">
        ...
        <div className="flex flex-col md:flex-row divide-y md:divide-y-0 md:divide-x divide-white/5 overflow-y-auto no-scrollbar">
           ...
           <div className="w-full md:w-5/12 p-4 space-y-4 shrink-0">
              {/* Complex Dynamics Board */}
              <div className="bg-black/40 p-3 rounded-xl border border-white/5 space-y-3">
                 <div className="flex justify-between items-center">
                    <h4 className="text-[7px] font-black text-gray-500 uppercase tracking-widest">Internal Dynamics</h4>
                 ...
                 <div className="space-y-2">
                    ...
                    <div>
                       <div className="flex justify-between text-[8px] font-black uppercase mb-1">
                          <span className="text-gray-500">System Fit</span>
                          <span className="text-green-500">{player.systemProficiency}%</span>
                       </div>
                       <div className="h-1 bg-white/5 rounded-full overflow-hidden">
                          <div className="h-full bg-green-500" style={{ width: `${player.systemProficiency}%` }}></div>
                       </div>
                    </div>
                 </div>
              </div>
              ...
              <div className="bg-black/20 p-3 rounded-xl border border-white/5">
                 <h4 className="text-[7px] font-black text-gray-500 uppercase tracking-widest mb-2">Performance Data</h4>
              ...
              <div className="pt-2">
                {player.isClient ? (
                  ...
                  >
                    {canSign ? 'APPROACH PLAYER' : `NEED REP ${reqRep}`}
                  </button>
                )}
              </div>
           </div>
           ...
           <div className="flex-1 p-4 bg-black/10">
              ...
              <div className="space-y-3">
                {[
                  ...
                  const cost = getTrainingCost(player.stats[attr.key]);
                  const canAfford = player.isClient && trainingPoints >= cost && player.stats[attr.key] < 99;
                  return (
                    <div key={attr.key} className="bg-white/5 p-2 rounded-xl border border-white/5 flex items-center justify-between">
                       <div className="flex-1 mr-4">
                          <div className="flex justify-between text-[8px] font-black uppercase mb-1">
                             ...
                             <span className="text-white">{player.stats[attr.key]}</span>
                          </div>
                          <div className="h-1 bg-black/40 rounded-full overflow-hidden">
                             <div className={`${attr.color} h-full`} style={{ width: `${player.stats[attr.key]}%` }}></div>
                          </div>
                       </div>
                       <button 
                         onClick={() => onTrainAttribute(player.id, attr.key)}
                         disabled={!canAfford}
                         className="h-8 w-12 rounded-lg bg-white text-black text-[9px] font-black flex items-center justify-center disabled:opacity-20 shadow-sm"
                       >
                          +1
                       </button>
                    </div>
